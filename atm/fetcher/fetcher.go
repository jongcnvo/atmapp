...
func (f *Fetcher) loop() {
	...
	for {
		...
		for !f.queue.Empty() {
			...
			hash := op.block.Hash()
			if number+uint64(maxUncleDist) < height || f.getBlock(hash) != nil {
				f.forgetBlock(hash)
				continue
			}
		...
		case notification := <-f.notify:
			...
			if notification.number > 0 {
				if dist := int64(notification.number) - int64(f.chainHeight()); dist < -int64(maxUncleDist) || dist > int64(maxQueueDist) {
...
func (f *Fetcher) enqueue(peer string, block *types.Block) {
	...
	if dist := int64(block.NumberU64()) - int64(f.chainHeight()); dist < -int64(maxUncleDist) || dist > int64(maxQueueDist) {

...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing ATMChain peer", "peer", id)

	// Unregister the peer from the downloader and ATMChain peer set
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("ATMChain peer connected", "name", p.Name())

	// Execute the ATMChain handshake
	td, head, genesis := pm.blockchain.Status()
	if err := p.Handshake(pm.networkId, td, head, genesis); err != nil {
		p.Log().Debug("ATMChain handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("ATMChain peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("ATMChain message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	case p.version >= atm1 && msg.Code == GetNodeDataMsg:
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err
	...
	case p.version >= atm1 && msg.Code == NodeDataMsg:
	...
	case p.version >= atm1 && msg.Code == GetReceiptsMsg:
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err
	...
	case p.version >= atm1 && msg.Code == ReceiptsMsg:
...
type NodeInfo struct {
	Network    uint64              `json:"network"`    // network ID (1=Universe)
